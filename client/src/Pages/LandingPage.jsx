...
function LandingPage() {
	...
	const Card = ({ card }) => {
		return (
		<div key={card.id} className="bg-white w-[350px] h-[250px] rounded-lg drop-shadow-lg m-3">
		...
		<Fragment>
			<div className="grid grid-cols-2 justify-evenly min-h-[40vh] py-5 my-2 items-center gap-4">
				<div className="col-span-1 flex-col justify-center items-center px-5">
					...
					transition={{staggerChildren:0.1, duration:0.9, type:"spring", stiffness:120}}
					...
					<h3 className="text-3xl py-2 mb-3 px-10 font-bold">
						...
						>Reuse</motion.span>, <motion.span className='text-yellow-300 inline-block'
						initial={{opacity:0, x:-10,y:10}}
						animate={{opacity:1, x:0,y:0}}
					...
					<p className="text-lg py-2 my-2 px-10 break-words font-semibold">
						{["Let's\u00a0", "make\u00a0", "the\u00a0", "planet\u00a0", "clean\u00a0", "and\u00a0", "population\u00a0", "healthy!"].map((el,index) => {
							return(
							<motion.span
								className='inline-block'
								key={index}
			...
			<div className="my-10 min-h-[50vh] overflow-x-hidden flex-col items-center justify-center">
				<h1 className="text-4xl font-semibold mx-4 mb-10 flex justify-center">What do they say?</h1>
				{/* <div><CardSlider /></div> */}
				<div  className="relative flex justify-center">
      <div className="flex items-center overflow-hidden">
        <motion.div ref={ref2} style={{ x }} className="flex justify-center w-[95vw] gap-4">
          {cards.map((card) => {
						...
						<div className="grid grid-cols-2 gap-8 sm:mt-4 sm:grid-cols-3 sm:gap-6">
							<div>
								...
								<Footer.LinkGroup col>
									...
									<Footer.Link href="/" className='text-gray-600'>Tailwind CSS</Footer.Link>
								</Footer.LinkGroup>
							</div>
							<div>
								<Footer.Title title="Follow us" className='text-gray-700' />
								<Footer.LinkGroup col>
									<Footer.Link href="/" className='text-gray-600'>Github</Footer.Link>
									<Footer.Link href="/" className='text-gray-600'>Discord</Footer.Link>
								</Footer.LinkGroup>
							</div>
							<div>
